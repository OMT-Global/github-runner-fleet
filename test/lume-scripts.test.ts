...
describe("Lume pool scripts", () => {
  test("creates and recycles cloned macOS VM slots", () => {
    ...
    expect(destroySlot).toContain('run_with_timeout "${LUME_VM_STOP_TIMEOUT_SECONDS:-120}" lume stop "${LUME_VM_NAME}"');
    expect(destroySlot).toContain('run_with_timeout "${LUME_VM_DELETE_TIMEOUT_SECONDS:-180}" lume delete "${LUME_VM_NAME}" --force');
    expect(destroySlot).toContain('terminate_tracked_process "${LUME_SLOT_VM_PID_FILE}" "lume run"');
    expect(destroySlot).toContain("cannot stall the pool");
    ...
    expect(runSlot).toContain("spawn_detached");
    expect(runSlot).toContain('echo "${bootstrap_pid}" > "${bootstrap_pid_file}"');
    expect(runSlot).toContain('watch_guest_session "${bootstrap_pid_file}" "${config_path}" "${env_path}" "${slot}"');
    expect(runSlot).toContain('upload_guest_file "${REPO_ROOT}/scripts/guest/listener-health-probe.sh" "${LUME_GUEST_PROBE_PATH}"');
    expect(runSlot).toContain("runner_zombie_recycled");
    expect(runSlot).toContain("runner_unreachable_recycled");
    expect(runSlot).toContain('terminate_tracked_process "${bootstrap_pid_file}" "lume ssh"');
    ...
    expect(reconcile).toContain('run_with_timeout "${LUME_VM_STOP_TIMEOUT_SECONDS:-120}" lume stop "${vm_name}"');
    expect(reconcile).toContain('run_with_timeout "${LUME_VM_DELETE_TIMEOUT_SECONDS:-180}" lume delete "${vm_name}" --force');
    ...
    const lumeLib = read("scripts/lume/lib.sh");
    expect(lumeLib).toContain("run_with_timeout()");
    expect(lumeLib).toContain("probe_guest_listener_health()");
    expect(lumeLib).toContain("fetch_queued_job_count()");
    expect(lumeLib).toContain("watch_guest_session()");
    expect(lumeLib).toContain('WATCHDOG_VERDICT="zombie"');
    expect(lumeLib).toContain('WATCHDOG_VERDICT="unreachable"');
    expect(lumeLib).toContain("lume-queued-jobs");
  ...
  test("bootstraps ephemeral macOS runners inside guest VMs", () => {
    ...
    expect(bootstrap).toContain("kill_stale_runner_processes");
    ...
    expect(probe).toContain("diag_age");
