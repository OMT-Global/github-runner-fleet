...
```bash
./scripts/build-image.sh ghcr.io/your-org/github-runner-fleet:0.2.3 --push
```
...
`scripts/lume/provision-base-vm.sh` also installs and verifies an available iOS Simulator runtime after Xcode first-launch setup. Re-run the provisioner whenever Xcode or the sealed base image changes; consumer jobs should verify the runtime, but should not need to download the multi-gigabyte platform into each ephemeral slot.
